...
class Directive implements IDirective<IScope,JQLite,IAttributes,IController[]> {
	...
	// 	let pulsarInfos = scope.$eval(attrs.pulsar);
	// 	//contenu des attrs
	...
	// 			let xPositions = {
	// 				left: elem.offset().left - (pulsarSize + pulsarMarge),
	// 				right: elem.offset().left + elem.width() + pulsarMarge,
	// 				center: elem.offset().left + (elem.width() / 2) - pulsarSize / 2
	// 			};

	// 			let yPositions = {
	// 				top: elem.offset().top,
	// 				bottom: elem.offset().top + elem.height() + pulsarMarge,
	// 				center: elem.offset().top + (elem.height() / 2) - pulsarSize / 2
	// 			};

	// 			if(pulsarInfos.position === 'top center'){
	// 				yPositions.top = elem.offset().top - pulsarSize - pulsarMarge;
	// 			}
