...
class Controller implements IController {
	...
	getEleve(index:number){
		if (index < this.eleves.length) {
			const eleve = this.eleves[index];
			this.contentTypes.forEach( (type:ContentType) => {
				this.getContent( type, eleve );
			});
		}
	}
	...
	openLightBox(contentType:any, eleve:any){
...
class Directive implements IDirective<IScope,JQLite,IAttributes,IController[]> {
    ...
    link(scope:IScope, elem:JQLite, attrs:IAttributes, controllers:IController[]|undefined): void {
		...
		TransportFrameworkFactory.instance().http.get('/sso/pronote')
		.then( (structures:any[]) => {
			...
			structures.forEach( structure => {
				...
				//FIXME if(!ctrl.myeleve && ctrl.eleves.length > 0){
					ctrl.getEleve(0);
				//}
			});
		})
		.catch( (e) => {
            ...
            ctrl.errorMsg = TransportFrameworkFactory.instance().http.latestResponse.statusText;
		});		
	}
}
